import React, { useState } from 'react';
import { NavLink, useLocation, useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import {
  LayoutDashboard,
  Boxes,
  Package,
  Truck,
  FileCheck2,
  ArrowDownLeft,
  ArrowUpRight,
  ArrowLeftRight,
  Sliders,
  AlertTriangle,
  Clock,
  FileSpreadsheet,
  ChevronDown,
  ChevronRight,
  LogOut,
  Warehouse,
  ShoppingBag,
  Activity,
  Calendar,
} from 'lucide-react';

interface StoreSidebarProps {
  mobileOpen: boolean;
  setMobileOpen: (open: boolean) => void;
}

export const StoreSidebar: React.FC<StoreSidebarProps> = ({ mobileOpen, setMobileOpen }) => {
  const { user, logout } = useAuth();
  const location = useLocation();
  const navigate = useNavigate();

  // Accordion state for "Inventory & Store Management"
  const [storeMenuOpen, setStoreMenuOpen] = useState(true);

  const handleLogout = () => {
    logout();
    navigate('/login');
  };

  const navItemClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 ${
      isActive
        ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20'
        : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
    }`;

  const subNavItemClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 pl-8 ${
      isActive
        ? 'bg-blue-50 text-blue-700 font-semibold border-l-2 border-blue-600'
        : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
    }`;

  const storeSubLinks = [
    { to: '/store/item-master', label: 'Item Master', icon: Package },
    { to: '/store/vendors', label: 'Vendor Management', icon: Truck },
    { to: '/store/purchase-orders', label: 'Purchase Orders', icon: ShoppingBag },
    { to: '/store/grn', label: 'Goods Receipt (GRN)', icon: FileCheck2 },
    { to: '/store/stock-inward', label: 'Stock Inward', icon: ArrowDownLeft },
    { to: '/store/stock-outward', label: 'Stock Outward', icon: ArrowUpRight },
    { to: '/store/stock-transfer', label: 'Stock Transfer', icon: ArrowLeftRight },
    { to: '/store/stock-adjustment', label: 'Stock Adjustment', icon: Sliders },
    { to: '/store/reorder-management', label: 'Reorder Management', icon: AlertTriangle },
    { to: '/store/batch-expiry', label: 'Batch & Expiry Tracking', icon: Clock },
    { to: '/store/reports', label: 'Inventory Reports', icon: FileSpreadsheet },
    { to: '/store/leave', label: 'Leave Management', icon: Calendar },
    { to: '/store/shift-roster', label: 'My Shift Roster', icon: Clock },
  ];

  return (
    <>
      {/* Mobile Backdrop */}
      {mobileOpen && (
        <div
          className="fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-xs lg:hidden"
          onClick={() => setMobileOpen(false)}
        />
      )}

      {/* Sidebar Drawer / Container */}
      <aside
        className={`fixed top-0 left-0 bottom-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 shrink-0 ${
          mobileOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'
        }`}
      >
        {/* Company Brand Logo Header */}
        <div className="h-20 px-5 flex items-center gap-3 border-b border-slate-100 shrink-0">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-cyan-500 flex items-center justify-center text-white shadow-md shadow-blue-500/20 shrink-0">
            <Activity className="w-5 h-5" />
          </div>
          <div className="min-w-0">
            <h1 className="text-base font-bold text-slate-900 truncate">AegisCare HMS</h1>
            <span className="inline-block text-[10px] font-semibold tracking-wider text-blue-600 uppercase">
              STORE PORTAL
            </span>
          </div>
        </div>

        {/* Navigation Body */}
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {/* Main Dashboard Link */}
          <NavLink
            to="/store/dashboard"
            end
            onClick={() => setMobileOpen(false)}
            className={navItemClass}
          >
            <LayoutDashboard className="w-4 h-4" />
            <span>Dashboard</span>
          </NavLink>

          {/* Expandable Group: Inventory & Store Management */}
          <div className="pt-1">
            <button
              onClick={() => setStoreMenuOpen(!storeMenuOpen)}
              className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold text-slate-800 hover:bg-slate-100 transition-colors cursor-pointer"
            >
              <div className="flex items-center gap-3">
                <Boxes className="w-4 h-4 text-blue-600" />
                <span>Inventory & Store Mgmt</span>
              </div>
              {storeMenuOpen ? (
                <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
              ) : (
                <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
              )}
            </button>

            {storeMenuOpen && (
              <div className="mt-1 space-y-0.5">
                {storeSubLinks.map((link) => {
                  const Icon = link.icon;
                  return (
                    <NavLink
                      key={link.to}
                      to={link.to}
                      onClick={() => setMobileOpen(false)}
                      className={subNavItemClass}
                    >
                      <Icon className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                      <span className="truncate">{link.label}</span>
                    </NavLink>
                  );
                })}
              </div>
            )}
          </div>
        </div>

        {/* User Card & Logout Footer */}
        <div className="p-3 border-t border-slate-100 bg-slate-50/50 shrink-0">
          <div className="flex items-center justify-between p-2 rounded-xl bg-white border border-slate-200/80 shadow-xs">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-8 h-8 rounded-lg bg-blue-100 text-blue-700 font-bold flex items-center justify-center text-xs shrink-0">
                {user?.name?.[0] || 'S'}
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold text-slate-900 truncate">{user?.name || (user?.username ? `User (${user.username})` : 'Store Manager')}</p>
                <p className="text-[10px] text-slate-500 truncate">{user?.branch || user?.department || 'Main Branch'}</p>
              </div>
            </div>
            <button
              onClick={handleLogout}
              title="Logout"
              className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-colors shrink-0 cursor-pointer"
            >
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        </div>
      </aside>
    </>
  );
};
